...
function go() {
  ...
  var embeds = document.querySelectorAll('object');

  console.log('Found ' + imgs.length + ' images ' + links.length + ' links ' + iframes.length + ' iframes ' + embeds.length + ' embeds');
  var results = [];
  var found = false;
  var el, res, uri, flashVars;
  ...
  for (ii = embeds.length; ii--;) {
    el = embeds[ii];
    flashVars = el.getAttribute('flashvars');
    console.log('Got flashVars: ' + flashVars);
    var matches = flashVars.match('clickTag=(.*?)&');
    
    if (matches.length > 0) {
      uri = decodeURIComponent(matches[1]);      
    console.log('Flash click uri: ' + uri);
    res = matchPattern(uri);
    if (res) {
      res.uri = uri;
      console.dir(res);
